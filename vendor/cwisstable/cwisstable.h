...
/// `CWISS_HAVE_NEON` is nonzero if we have NEON support.
///
/// `-DCWISS_HAVE_NEON` can be used to override it; it is otherwise detected
/// via the usual non-portable feature-detection macros.
#ifndef CWISS_HAVE_NEON
  #if defined(__ARM_NEON) || defined(__ARM_NEON__)
    #define CWISS_HAVE_NEON 1
  #else
    #define CWISS_HAVE_NEON 0
  #endif
#endif
...
typedef struct {
  ...
  /// The ctx parameter allows passing an arena or other allocation context.
  void* (*alloc)(size_t size, size_t align, void* ctx);
  ...
  /// The ctx parameter allows passing an arena or other allocation context.
  void (*free)(void* array, size_t size, size_t align, void* ctx);
} CWISS_AllocPolicy;
...
typedef struct {
  ...
  size_t growth_left_;
  /// Custom arena allocator (may be null for default malloc/free).
  CWISS_Arena arena_;
} CWISS_RawTable;

static inline void* CWISS_TableAlloc(const CWISS_Policy* policy,
                                     const CWISS_RawTable* self,
                                     size_t size, size_t align) {
  return policy->alloc->alloc(size, align, self->arena_.ctx);
}

static inline void CWISS_TableFree(const CWISS_Policy* policy,
                                   const CWISS_RawTable* self,
                                   void* ptr, size_t size, size_t align) {
  policy->alloc->free(ptr, size, align, self->arena_.ctx);
}
...
static inline void CWISS_RawTable_DestroySlots(const CWISS_Policy* policy,
  ...
  CWISS_TableFree(policy, self, self->ctrl_,
...
static inline void CWISS_RawTable_Resize(const CWISS_Policy* policy,
  ...
  if (old_capacity) {
    ...
    CWISS_TableFree(policy, self, old_ctrl,
...
static void CWISS_RawTable_DropDeletesWithoutResize(const CWISS_Policy* policy,
  ...
  // alloca...
  void* slot = CWISS_TableAlloc(policy, self, policy->slot->size, policy->slot->align);
...
#undef CWISS_ProbeSeq_Start_index
  ...
  CWISS_RawTable_ResetGrowthLeft(policy, self);
  CWISS_TableFree(policy, self, slot, policy->slot->size, policy->slot->align);
  // infoz().RecordRehash(total_probe_length);
}
...
/// Creates a new empty table with the given capacity and optional arena.
...
static inline CWISS_RawTable CWISS_RawTable_dup(const CWISS_Policy* policy,
                                                const CWISS_RawTable* self) {
  CWISS_RawTable copy = CWISS_RawTable_new(policy, 0, self->arena_);
...
#define CWISS_DECLARE_COMMON_(HashSet_, Type_, Key_, kPolicy_)                 \
  ...
  static inline void HashSet_##_dump(const HashSet_* self) {                   \
    CWISS_RawTable_dump(&kPolicy_, &self->set_);                               \
                                                                               ...
                                                                               \
  static inline HashSet_ HashSet_##_new(size_t bucket_count, void* arena) {    \
    CWISS_Arena a = {arena};                                                   \
    return (HashSet_){CWISS_RawTable_new(&kPolicy_, bucket_count, a)};         \
  }                                                                            \
