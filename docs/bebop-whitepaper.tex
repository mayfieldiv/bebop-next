...
This paper presents Bebop, a schema-driven binary format where every type has a fixed wire size. The schema compiler generates encode and decode routines without data-dependent branches. On modern hardware, throughput is limited by memory bandwidth rather than CPU compute. The generated code does little more than pointer arithmetic and bounds checking.
...
\item Empirical comparison against Protocol Buffers, MessagePack, and simdjson across 19 workloads representing ML inference, event streaming, and recursive data structures
\item A schema language with compile-time extensibility through embedded Lua scripting
\item Reference implementations in C demonstrating terabyte-per-second decode throughput
\end{enumerate}
\end{minipage}
...
Messages support forward and backward compatibility. Old code ignores unknown tags; new code uses defaults for missing tags. Adding a field with a new tag number is always safe. Removing a field is safe if readers handle missing tags. Reusing a tag number with a different type breaks compatibility.
...
Table~\ref{tab:decode} presents decode latency and throughput across all workloads. Bebop decoded faster than Protocol Buffers on all 19 workloads and faster than MessagePack on 18 of 19.
...
Table~\ref{tab:throughput} shows decode throughput across workloads.

\begin{table}[ht]
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Workload} & \textbf{Throughput} & \textbf{Notes} \\
\midrule
TensorShardLarge & 9.58 TB/s & 64KB bfloat16 array \\
Embedding1536 & 1.10 TB/s & 1536-dim embedding vector \\
EmbeddingBatch & 964.13 GB/s & Batch of 32 embeddings \\
EventLarge & 644.35 GB/s & 4KB payload, minimal framing \\
Embedding768 & 534.90 GB/s & 768-dim embedding vector \\
InferenceResponse & 355.51 GB/s & Complete inference result \\
OrderLarge & 212.50 GB/s & Nested arrays \\
ChunkedText & 48.56 GB/s & 1000 text spans \\
DocumentLarge & 41.96 GB/s & 2KB text with metadata \\
PersonMedium & 30.88 GB/s & Typical API record \\
\bottomrule
\end{tabular}
\caption{Bebop decode throughput on selected workloads.}
\label{tab:throughput}
\end{table}

The M3 Ultra delivers 819 GB/s memory bandwidth~\cite{m3-ultra}. Throughput above this (e.g., TensorShardLarge at 9.58 TB/s) indicates cache-resident data from benchmark iterations. For DRAM-fetched data, decode throughput tracks memory bandwidth. Bebop's decode path does minimal computation: bounds checking, pointer arithmetic, occasional type conversion. Most ``decode'' operations are pointer assignments.

Figure~\ref{fig:bandwidth-utilization} shows measured throughput as a fraction of peak cache/memory bandwidth at each level of the hierarchy.
...
SIMD could reduce per-field overhead further, and systems with higher memory bandwidth would see proportionally higher throughput.

\subsection{Encode Performance}

Encode margins are smaller than decode. Bebop beat Protocol Buffers on all 22 workloads (1.4--12.6$\times$) and MessagePack on 15 of 19 (1.2--19.4$\times$). MessagePack was faster on JsonSmall, JsonLarge, ChunkedText, and DocumentLarge.
...
\toprule
\textbf{Workload} & \textbf{Bebop} & \textbf{Protocol Buffers} & \textbf{Bebop + brotli} \\
 & (bytes) & (bytes) & (bytes) \\
\midrule
\multicolumn{4}{l}{\textit{API payloads (small integer fields)}} \\
PersonSmall & 28 & 19 & --- \\
PersonMedium & 130 & 124 & 101 \\
OrderSmall & 76 & 34 & 62 \\
OrderLarge & 1,240 & 423 & 514 \\
\midrule
\multicolumn{4}{l}{\textit{Event payloads (byte arrays)}} \\
EventSmall & 42 & 29 & --- \\
EventLarge & 4,170 & 4,158 & --- \\
\midrule
\multicolumn{4}{l}{\textit{ML inference (bfloat16 arrays)}} \\
Embedding768 & 1,556 & 1,573 & 1,167 \\
Embedding1536 & 3,092 & 3,109 & 2,219 \\
TensorShardSmall & 2,132 & 2,108 & 1,574 \\
TensorShardLarge & 65,624 & 65,605 & 44,711 \\
\bottomrule
\end{tabular}
\caption{Wire size in bytes. --- indicates brotli increased size.}
\label{tab:wire-size}
\end{table}

The results confirm the predictions from Equation~\ref{eq:varint-expected}. OrderLarge contains arrays of 100 small integers; varint encodes these in 1--2 bytes each while fixed-width requires 4--8 bytes, producing a 2.93$\times$ size difference.

EventLarge is dominated by a 4KB random byte payload; neither format compresses it. The ML workloads write bfloat16 arrays as raw bytes with a length prefix; the formats produce nearly identical uncompressed output.

Compression changes the tradeoff. TensorShardLarge compresses to 44,711 bytes (brotli), 32\% smaller than uncompressed Protocol Buffers. For ML workloads, Bebop with compression produces smaller wire sizes than Protocol Buffers alone.
