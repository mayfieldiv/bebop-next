...
- [ ] Bulk scalar array optimization (per-element loop currently) → FD-010
...
- [x] Runtime has generic `read_fixed_array`/`write_fixed_array` for all scalar types (FD-015)
...
- [/] Mutable struct (`mut`) — intentionally not distinguished in Rust (FD-011, Closed)
...
- [/] Deprecated fields still encoded (C skips them) → FD-002
...
- [ ] Service definition generation → FD-001
...
- [x] Top-level definitions default to exported → `pub` (FD-021)
- [x] `local` definitions → `pub(crate)` (FD-021)
- [x] Nested definitions default to local → uses `visibility` field from descriptor (FD-021)
- [x] `export` on nested → uses `visibility` field from descriptor (FD-021)
...
- [x] Insertion point markers (`// @@bebop_insertion_point(...)`) (FD-017)
...
- [x] `Visibility` enum (DEFAULT, EXPORT, LOCAL)

---

## RPC Wire Protocol (RPC.md)

These are relevant when services are implemented → FD-001
...
- [ ] Per-type reflection metadata → FD-009
- [ ] Type name, FQN, kind
...
- [x] `PartialEq`/`Eq`/`Hash` derives on eligible types (FD-014)
- [x] Serde integration (FD-003)
- [x] Timestamp/Duration newtypes (FD-019)
- [x] UUID crate integration (FD-020)
- [x] Dead code cleanup (FD-013)
- [x] Rename `_cow` suffixed functions (FD-018)
- [x] Name-collision hardening (FD-016)
- [x] Insertion points (FD-017)
- [x] Visibility support (FD-021)
