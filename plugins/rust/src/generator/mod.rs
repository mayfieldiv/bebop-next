...
/// Tracks which optional runtime types a schema uses.
...
impl TypeUsage {
  /// Scan all definitions in a schema and record which optional types are used.
  fn from_schema(schema: &SchemaDescriptor) -> Self {
    let mut usage = Self::default();
    for def in schema.definitions.as_deref().unwrap_or(&[]) {
      usage.scan_definition(def);
    }
    usage
  }

  fn scan_definition(&mut self, def: &DefinitionDescriptor) {
    // Struct fields
    if let Some(s) = &def.struct_def {
      for f in s.fields.as_deref().unwrap_or(&[]) {
        if let Some(ty) = &f.r#type {
          self.scan_type(ty);
        }
      }
    }
    // Message fields
    if let Some(m) = &def.message_def {
      for f in m.fields.as_deref().unwrap_or(&[]) {
        if let Some(ty) = &f.r#type {
          self.scan_type(ty);
        }
      }
    }
    // Union branches: inline definitions are in def.nested (handled below),
    // type-ref branches reference definitions scanned at their own level.
    // Service methods
    if let Some(s) = &def.service_def {
      for method in s.methods.as_deref().unwrap_or(&[]) {
        if let Some(ty) = &method.request_type {
          self.scan_type(ty);
        }
        if let Some(ty) = &method.response_type {
          self.scan_type(ty);
        }
      }
    }
    // Nested definitions
    for nested in def.nested.as_deref().unwrap_or(&[]) {
      self.scan_definition(nested);
    }
    // Const definitions have a type that may reference uuid/half.
    if let Some(c) = &def.const_def {
      if let Some(ty) = &c.r#type {
        self.scan_type(ty);
      }
    }
  }

  fn scan_type(&mut self, ty: &TypeDescriptor) {
    match ty.kind {
      Some(TypeKind::Uuid) => self.uses_uuid = true,
      Some(TypeKind::Float16) | Some(TypeKind::Bfloat16) => self.uses_half = true,
      _ => {}
    }
    if let Some(elem) = &ty.array_element {
      self.scan_type(elem);
    }
    if let Some(elem) = &ty.fixed_array_element {
      self.scan_type(elem);
    }
    if let Some(key) = &ty.map_key {
      self.scan_type(key);
    }
    if let Some(val) = &ty.map_value {
      self.scan_type(val);
    }
  }
}
...
impl RustGenerator {
  ...
  pub fn generate(
    ...
    let definitions = schema.definitions.as_deref().unwrap_or(&[]);
    let type_usage = TypeUsage::from_schema(schema);
    ...
    output.push_str("use bebop_runtime::{BebopDecode, BebopDuration, BebopEncode, BebopFlags, BebopReader, BebopTimestamp, BebopWriter, DecodeError};\n");
    if type_usage.uses_half {
      output.push_str("use bebop_runtime::{bf16, f16};\n");
    }
    if type_usage.uses_uuid {
      output.push_str("use bebop_runtime::Uuid;\n");
    }
