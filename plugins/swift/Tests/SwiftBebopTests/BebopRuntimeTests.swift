...
@Test func arrayLength() throws {
    let r = try roundTrip {
        $0.writeArrayLength(42)
    } read: {
        try $0.readArrayLength()
    }
    #expect(r == 42)
}
...
@Test func skip() throws {
    var writer = BebopWriter()
    writer.writeUInt32(0xAAAA_AAAA)
    writer.writeUInt32(0xBBBB_BBBB)
    let v = try writer.withUnsafeBytes { buf -> UInt32 in
        var reader = BebopReader(data: buf)
        try reader.skip(4)
        return try reader.readUInt32()
    }
    #expect(v == 0xBBBB_BBBB)
}
...
@Test func mixedTypes() throws {
    ...
    writer.writeUInt64(0xDEAD_BEEF)
...
@Test func unalignedReads() throws {
    ...
    writer.writeUInt64(0xCAFE_BABE_1234_5678)
    ...
    try writer.withUnsafeBytes { buf in
        ...
        #expect(vByte == 0xFF)
        #expect(vU16 == 0x1234)
        #expect(vU32 == 0xDEAD_BEEF)
        #expect(vU64 == 0xCAFE_BABE_1234_5678)
    }
}
