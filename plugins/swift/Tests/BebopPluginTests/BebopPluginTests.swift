...
@Test func methodTypeRoundTrip() throws {
    for mt: MethodType in [.unary, .serverStream, .clientStream, .duplexStream] {
...
@Test func definitionDescriptorEnum() throws {
    let def = DefinitionDescriptor(
        ...
        enumDef: EnumDef(baseType: .uint32, members: [
...
@Test func definitionDescriptorWithDocAndDecorators() throws {
    let def = DefinitionDescriptor(
        ...
        decorators: [
            DecoratorUsage(fqn: "deprecated", args: [
...
@Test func diagnosticRoundTrip() throws {
    ...
    span[0] = 1; span[1] = 5; span[2] = 1; span[3] = 20
    let d = Diagnostic(
        severity: .warning,
