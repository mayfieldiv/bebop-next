...
public struct FutureDispatcher<Channel: BebopChannel>: Sendable {
  ...
  public init(channel: Channel, maxCompletedResults: Int = 10_000) {
    self.init(
      channel: channel,
  ...
  public func future<T: BebopRecord>(id: BebopUUID, as type: T.Type) -> BebopFuture<T> {
    BebopFuture(
      id: id,
      resolver: resolver,
...
extension BebopChannel {
  ...
  public func makeFutureResolver(maxCompletedResults: Int = 10_000) -> FutureResolver {
    FutureResolver(
      maxCompletedResults: maxCompletedResults,
      sendCancel: { [self] payload in
        _ = try await self.unary(
  ...
  public func makeFutureDispatcher(maxCompletedResults: Int = 10_000) -> FutureDispatcher<Self> {
