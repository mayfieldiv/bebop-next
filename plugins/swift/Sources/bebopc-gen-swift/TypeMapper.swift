...
enum TypeMapper {
    ...
    static func writeExpression(for type: TypeDescriptor, value: String, writer w: String = "writer") throws -> String {
        guard let kind = type.kind else {
            throw CodegenError.malformedType("missing type kind")
        }
        switch kind {
    ...
    static func sizeExpression(for type: TypeDescriptor, value: String) throws -> String {
        ...
        case .map:
            ...
            return "(4 + \(value).reduce(0) { _acc, _kv in let (\(kBind), \(vBind)) = _kv; return _acc + \(keyExpr) + \(valExpr) })"
        case .defined:
    ...
    static func fixedSize(for type: TypeDescriptor) -> Int? {
        ...
        if kind == .fixedArray {
            guard let elem = type.fixedArrayElement,
                  ...
                  let count = type.fixedArraySize else { return nil }
