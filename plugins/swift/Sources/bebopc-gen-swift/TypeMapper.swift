...
enum TypeMapper {
    ...
    static func unshadow(_ name: String) -> String {
        shadowedNames.contains(name) ? "Swift.\(name)" : name
    }
    ...
    static func swiftType(for type: TypeDescriptor) throws -> String {
        ...
        switch kind {
        case .bool: return unshadow("Bool")
        ...
        case .string: return unshadow("String")
        ...
        case .fixedArray:
            ...
            return try "InlineArray<\(size), \(swiftType(for: elem))>"
        case .map:
            ...
            return try "[\(swiftType(for: key)): \(swiftType(for: val))]"
        case .defined:
