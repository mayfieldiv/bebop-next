...
enum GenerateUnion {
    static func generate(_ def: DefinitionDescriptor, nested: [String] = [], options: GeneratorOptions) throws -> [String] {
        ...
        var decodeSwitchLines: [String] = ["switch disc {"]
        for branch in branches {
        ...
        // encodedSize
        var sizeSwitchLines: [String] = ["switch self {"]
        for branch in branches {
            let caseName = try branchCaseName(branch, unionName: defName)
        ...
        // CodingKeys
        body.append("enum CodingKeys: String, CodingKey { case discriminator, value }")
        ...
        var codableDecodeSwitchLines: [String] = ["switch disc {"]
        for branch in branches {
            guard let disc = branch.discriminator else {
                throw CodegenError.malformedDefinition("union '\(defName)' branch missing discriminator")
            }
            ...
            codableDecodeSwitchLines.append("    self = .\(caseName)(try container.decode(\(typeName).self, forKey: .value))")
        }
        codableDecodeSwitchLines.append("default:")
        codableDecodeSwitchLines.append("    let data = try container.decode([UInt8].self, forKey: .value)")
        ...
        // encode(to encoder:)
        var codableEncodeBody: [String] = ["var container = encoder.container(keyedBy: CodingKeys.self)"]
        var codableEncodeSwitchLines: [String] = ["switch self {"]
        for branch in branches {
            guard let disc = branch.discriminator else {
                throw CodegenError.malformedDefinition("union '\(defName)' branch missing discriminator")
            }
            ...
            codableEncodeSwitchLines.append("    try container.encode(UInt8(\(disc)), forKey: .discriminator)")
        ...
        body.append(
            "\(vis)static let bebopReflection = BebopTypeReflection(name: \(quoted(defName)), fqn: \(quoted(defFqn)), kind: .union, detail: .union(UnionReflection(branches: [\(try reflectionBranches(branches, unionName: defName))])))"
        )
