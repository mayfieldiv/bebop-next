...
enum GenerateEnum {
    static func generate(_ def: DefinitionDescriptor, options: GeneratorOptions) throws -> [String] {
        ...
        if isFlags {
            return try generateFlags(
                def, members: members, name: name, vis: vis, baseKind: baseKind, baseType: baseType,
                readMethod: readMethod, writeMethod: writeMethod
            )
        } else {
            return try generateRegularEnum(
                def, members: members, name: name, vis: vis, baseKind: baseKind, baseType: baseType,
                readMethod: readMethod, writeMethod: writeMethod
            )
        }
    ...
    private static func generateRegularEnum(
        ...
        let memberDecls = try members.map {
            member -> (caseName: String, literal: String, prefix: String) in
        ...
        try body.append(
            ...
            \(vis)static let bebopReflection = BebopTypeReflection(
                ...
                detail: .enum(
                    EnumReflection(
                        members: [
            \(indent(reflectionMembers(members, enumName: defName), 4))
                        ],
                        isFlags: false
                    )
                )
        ...
        return [
            "\(prefix)\(vis)struct \(name): RawRepresentable, Sendable, Hashable, BebopRecord, BebopReflectable {\n\(bodyStr)\n}",
        ]
    }
    ...
    private static func generateFlags(
        ...
        let memberDecls = try members.map {
            member -> (caseName: String, value: UInt64, literal: String, prefix: String) in
            guard let mName = member.name else {
        ...
        let bodyStr = body.map { indent($0) }.joined(separator: "\n\n")
        return [
            "\(prefix)\(vis)struct \(name): OptionSet, Sendable, BebopRecord, BebopReflectable {\n\(bodyStr)\n}",
        ]
    }

    private static func enumMemberLiteral(_ value: UInt64, baseKind: TypeKind) -> String {
        switch baseKind {
        case .int8: String(Int8(bitPattern: UInt8(truncatingIfNeeded: value)))
        case .int16: String(Int16(bitPattern: UInt16(truncatingIfNeeded: value)))
        case .int32: String(Int32(bitPattern: UInt32(truncatingIfNeeded: value)))
        case .int64: String(Int64(bitPattern: value))
        default: String(value)
        }
    }

    private static func reflectionMembers(_ members: [EnumMemberDescriptor], enumName: String) throws
        -> String
    {
